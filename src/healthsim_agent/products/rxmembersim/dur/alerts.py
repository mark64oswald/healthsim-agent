...
Ported from: healthsim-workspace/packages/rxmembersim/src/rxmembersim/dur/alerts.py
"""
...
class DURAlertSummary(BaseModel):
    ...
    claim_id: str
    total_alerts: int
    level_1_alerts: int = 0
    level_2_alerts: int = 0
    level_3_alerts: int = 0
...
class DURAlertFormatter:
    ...
    def format_for_display(self, alert: DURAlert) -> str:
        ...
        severity = self.SEVERITY_MAP.get(alert.clinical_significance, "UNKNOWN")
        alert_type = self.TYPE_MAP.get(alert.alert_type, alert.alert_type.value)
        lines = [f"[{severity}] {alert_type}", f"  Drug: {alert.drug1_name}"]
        if alert.drug2_name:
    ...
    def format_for_ncpdp(self, alert: DURAlert) -> dict:
        ...
        return {
            ...
            "previous_fill_date": alert.previous_fill_date.strftime("%Y%m%d") if alert.previous_fill_date else "",
            "database_indicator": "1",
    ...
    def create_summary(
        self, claim_id: str, alerts: list[DURAlert],
        override_provided: bool = False, override_codes: list[str] | None = None,
    ) -> DURAlertSummary:
        """Create summary of alerts for a claim."""
        level_1 = sum(1 for a in alerts if a.clinical_significance == ClinicalSignificance.LEVEL_1)
        level_2 = sum(1 for a in alerts if a.clinical_significance == ClinicalSignificance.LEVEL_2)
        level_3 = sum(1 for a in alerts if a.clinical_significance == ClinicalSignificance.LEVEL_3)
        requires_override = level_1 > 0
        can_proceed = True
        rejection_reason = None
        if level_1 > 0 and not override_provided:
            can_proceed = False
            rejection_reason = f"{level_1} major DUR alert(s) require pharmacist override"
        return DURAlertSummary(
            claim_id=claim_id, total_alerts=len(alerts),
            level_1_alerts=level_1, level_2_alerts=level_2, level_3_alerts=level_3,
            alerts=alerts, requires_override=requires_override,
            override_provided=override_provided, override_codes=override_codes or [],
            can_proceed=can_proceed, rejection_reason=rejection_reason,
        )
...
class DUROverrideManager:
    ...
    def create_override(
        self, alert: DURAlert, professional_service: str, result_of_service: str,
        pharmacist_name: str | None = None, notes: str | None = None,
    ) -> DUROverride:
        """Create an override for an alert."""
        return DUROverride(
            alert_type=alert.alert_type, reason_for_service=alert.reason_for_service,
            professional_service=professional_service, result_of_service=result_of_service,
            pharmacist_name=pharmacist_name, override_date=date.today(), notes=notes,
        )

    def apply_override_to_alert(self, alert: DURAlert, override: DUROverride) -> DURAlert:
        """Apply override codes to an alert."""
        return DURAlert(
            alert_type=alert.alert_type, clinical_significance=alert.clinical_significance,
            drug1_ndc=alert.drug1_ndc, drug1_name=alert.drug1_name, drug1_gpi=alert.drug1_gpi,
            drug2_ndc=alert.drug2_ndc, drug2_name=alert.drug2_name, drug2_gpi=alert.drug2_gpi,
            ...
            days_early=alert.days_early, previous_fill_date=alert.previous_fill_date,
        )


__all__ = [
    "DURAlertSummary", "DUROverride",
    "DURAlertFormatter", "DUROverrideManager",
]
