...
Ported from: healthsim-workspace/packages/patientsim/src/patientsim/formats/export.py
"""
...
def patients_to_csv(patients: list) -> str:
    ...
    fields = [
        ...
        "street", "city", "state", "zip_code", "race", "language", "ssn",
    ]
...
def encounters_to_csv(encounters: list) -> str:
    ...
    fields = [
        ...
        "chief_complaint", "discharge_disposition",
    ]
    ...
    for encounter in encounters:
        row = {
            ...
            "class_code": encounter.class_code.value if hasattr(encounter.class_code, "value") else str(encounter.class_code),
            "status": encounter.status.value if hasattr(encounter.status, "value") else str(encounter.status),
            "admission_time": encounter.admission_time.isoformat() if encounter.admission_time else "",
            "discharge_time": encounter.discharge_time.isoformat() if encounter.discharge_time else "",
...
def diagnoses_to_csv(diagnoses: list) -> str:
    ...
    fields = [
        "diagnosis_id", "patient_mrn", "encounter_id", "code", "code_system",
        "description", "type", "onset_date", "resolved_date", "is_principal",
    ]
    ...
    for dx in diagnoses:
        row = {
            ...
            "onset_date": dx.onset_date.isoformat() if hasattr(dx, "onset_date") and dx.onset_date else "",
            "resolved_date": dx.resolved_date.isoformat() if hasattr(dx, "resolved_date") and dx.resolved_date else "",
...
def medications_to_csv(medications: list) -> str:
    ...
    fields = [
        "medication_id", "patient_mrn", "name", "code", "code_system",
        "dose", "dose_unit", "route", "frequency", "start_date", "end_date", "status",
    ]
    ...
    for med in medications:
        row = {
            ...
            "start_date": med.start_date.isoformat() if hasattr(med, "start_date") and med.start_date else "",
            "end_date": med.end_date.isoformat() if hasattr(med, "end_date") and med.end_date else "",
...
def labs_to_csv(labs: list) -> str:
    ...
    fields = [
        "result_id", "patient_mrn", "encounter_id", "test_code", "test_name",
        "value", "units", "reference_range", "abnormal_flag", "result_time", "status",
    ]
    ...
    for lab in labs:
        row = {
            ...
            "result_time": lab.result_time.isoformat() if hasattr(lab, "result_time") and lab.result_time else "",
...
def vitals_to_csv(vitals: list) -> str:
    ...
    fields = [
        "vital_id", "patient_mrn", "encounter_id", "observation_time",
        "temperature", "heart_rate", "respiratory_rate",
        "systolic_bp", "diastolic_bp", "spo2", "weight_kg", "height_cm",
    ]
    ...
    for vital in vitals:
        row = {
            ...
            "observation_time": vital.observation_time.isoformat() if hasattr(vital, "observation_time") and vital.observation_time else "",
