# HealthSim Agent — Full Autoresearch Sweep
...
**Step 5: Test**
```bash
source .venv/bin/activate && pytest tests/ -v --tb=short -x
```
...
(e.g., patientsim/diabetes-management.md). Move detailed tables,
worked examples, and protocol descriptions there. Replace with
concise pointers: "See diabetes-management.md for protocol."

**Add high-value content in freed space:**
Once a skill is trimmed, use the freed space for safety guardrails,
negative examples, or edge case handling — the gaps the eval tests.
...
membersim            refactored     moved claim detail to sub-skills  2403 → 1850
rxmembersim          improved       added safety guardrails       2484 → 2490
...
```

Include word counts before and after for each skill touched.
